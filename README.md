...
> 当前版本：**v1.1.0** · 平台：**Windows 10/11 x64** · 许可证：**MIT**
...
- 本地 JSON 原子写入、上一版本回退、手动备份与 Markdown 导出
- 可选开机自启动
...
2. 下载 `个人工作面板-v1.1.0-windows-x64.exe`。
...
浏览器预览模式（不含托盘、全局快捷键、本地备份与导出等原生能力）：
...
5. 可在设置中切换开机启动、创建数据备份。
6. 已完成项目整体只读，但项目总结仍可继续补充；项目可导出为 Markdown。

## 数据与隐私

- 应用不提供账号、云同步、遥测或网络后端。
- 业务数据与粘贴图片保存在当前 Windows 用户的 Tauri 应用数据目录中，不会写入源码目录。
- 备份和 Markdown 导出只会写入用户通过系统对话框选择的位置。
